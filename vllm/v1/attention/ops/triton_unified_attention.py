...
# fardin_patch (LMI v27): tuned unified-attention config lookup. Applied AFTER
# PR#3's rewrite so the tuned-config override wraps PR#3's tile_size logic
# (per ADVERSARIAL_REVIEW.md line 88). On miss, preserves upstream behavior.
from vllm.v1.attention.ops.unified_attention_configs import (
...
def kernel_unified_attention(
    ...
    # [upstream 0.23.0 params — kept]
    USE_TD: tl.constexpr = False,
    USE_TD_QO: tl.constexpr = False,
    Q_IS_FP8: tl.constexpr = False,
    # OP-003 (LMI v27, PR#3, default-OFF): when True (3D + sliding-window +
    # spec-decode regime), the ``NUM_SEGMENTS_PER_SEQ`` parallel-softmax
    # segments tile only the sliding window's tile range rather than the full
    # sequence, so segment parallelism is not collapsed to 1/NSEG at long
    # context. Default False preserves the full-sequence (global / existing)
    # segmentation byte-for-byte. [PR#3 param — added additively after upstream]
...
def unified_attention(
    ...
    # fardin_patch (LMI v27): tuned-config consultation. Runs AFTER all of
    # PR#3's tile_size / num_warps logic above (OP-033 tile widening, OP-017/019
    # num_warps), so on a tuned-config HIT it WRAPS (overrides) PR#3's values —
    # the wrapping direction mandated by ADVERSARIAL_REVIEW.md line 88. On MISS
    # (_tuned_cfg is None) it is a no-op and PR#3's / upstream's values stand.
    _q_dtype_str = str(q.dtype).removeprefix("torch.")
    _tuned_cfg = get_unified_attention_config(
        head_size=head_size,
    ...
    if _tuned_cfg is not None:
        tile_size = int(_tuned_cfg.get("TILE_SIZE", tile_size))
        if "num_warps" in _tuned_cfg:
            _op017_kernel_kwargs["num_warps"] = int(_tuned_cfg["num_warps"])
        if "num_stages" in _tuned_cfg:
            _op017_kernel_kwargs["num_stages"] = int(_tuned_cfg["num_stages"])
    ...
    kernel_unified_attention[grid](
        ...
        # upstream 0.23.0 kwargs — kept
        USE_TD=use_td,
        USE_TD_QO=use_td_qo,
        # PR#3 kwargs — added additively (match the WINDOW_SEG_3D / OP-017
        # params unioned into the kernel signature above)
